...
public class FingerprintDialog extends DialogFragment implements FingerprintHandler.Callback {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        TextView tvTitle = getDialog().findViewById(android.R.id.title);
        tvTitle.setTextSize(19);
        tvTitle.setGravity(Gravity.CENTER);
        tvTitle.setText(this.dialogTitle);
        if(secondTextButton != null && secondTextButton != "" ) {
            mSecondButton.setVisibility(View.VISIBLE);
            mSecondButton.setText(secondTextButton);
        } else {
            mSecondButton.setVisibility(View.GONE);
        }
        getDialog().getWindow().setBackgroundDrawableResource(R.drawable.border_layout);
    ...
    public void setAuthConfig(final ReadableMap config) {
        ...
        if (config.hasKey("secondButtonText")) {
            this.secondTextButton = config.getString("secondButtonText");
        }
    }
    ...
    public void onClickSecondButton() {
        this.isAuthInProgress = false;
        this.mFingerprintHandler.endAuth();
        this.dialogCallback.onClickSecondButton();
        dismiss();
    }
}
